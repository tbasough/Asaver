    ...
    public function display_field($data = array(), $post = null, $echo = true)
    {
        ...
        switch ($field['type']) {
            ...
            case 'text':
                ...
                break;
            case 'url':
                $html .= '<input id="' . esc_attr($field['id']) . '" type="url" name="' . esc_attr($option_name) . '" placeholder="' . esc_attr($field['placeholder']) . '" value="' . esc_attr($data) . '" />' . "\n";
                break;
            case 'email':
                $html .= '<input id="' . esc_attr($field['id']) . '" type="email" name="' . esc_attr($option_name) . '" placeholder="' . esc_attr($field['placeholder']) . '" value="' . esc_attr($data) . '" />' . "\n";
                break;
            case 'text_disabled':
                ...
                break;
            case 'text_disabled_none':
                $html .= '<input type="text" placeholder="' . esc_attr($field['placeholder']) . '" value="' . esc_attr($field['default']) . '" disabled/>' . "\n";
                break;
            case 'password':
            case 'number':
            case 'hidden':
            ...
            case 'checkbox_system':
                $html .= $field['condition'] ? '<p style="color:green">OK</p>' : '<p style="color:red">Fail</p>' . "\n";
                break;
            case 'info_text':
                $html .= '<p>' . $data . '</p>' . "\n";
                break;
            case 'latest_downloads':
                $downloads = json_decode($data, true);
                ...
                if (!empty($downloads) && is_array($downloads)) {
                    foreach ($downloads as $download) {
                        ...
                        if( is_array( $download ) ){
                            ...
                            $image = '<img style="max-width:20vh" src="' . $download['thumbnail'] . '">';
                            $html .= '<tr><td>' . $image . '</td><td><a href="' . $download['url'] . '" target="_blank">' . $download['title'] . '</a></td><td>' . $download['source'] . '</td><td>' . $download['clientIp'] . '</td></tr>';
            ...
            case 'image':
                ...
                $html .= '<input id="' . $option_name . '_button" type="button" data-uploader_title="' . __('Upload an image', 'asr') . '" data-uploader_button_text="' . __('Use image', 'asr') . '" class="image_upload_button button" value="' . __('Upload new image', 'asr') . '" />' . "\n";
                $html .= '<input id="' . $option_name . '_delete" type="button" class="image_delete_button button" value="' . __('Remove image', 'asr') . '" />' . "\n";
