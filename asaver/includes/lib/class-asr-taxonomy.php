...
class ASR_Taxonomy {
	...
	public function register_taxonomy() {
		...
		$labels = array(
			...
			'update_item'                => sprintf( __( 'Update %s', 'asr' ), $this->single ),
			'add_new_item'               => sprintf( __( 'Add New %s', 'asr' ), $this->single ),
			'new_item_name'              => sprintf( __( 'New %s Name', 'asr' ), $this->single ),
			'parent_item'                => sprintf( __( 'Parent %s', 'asr' ), $this->single ),
			'parent_item_colon'          => sprintf( __( 'Parent %s:', 'asr' ), $this->single ),
			'search_items'               => sprintf( __( 'Search %s', 'asr' ), $this->plural ),
			'popular_items'              => sprintf( __( 'Popular %s', 'asr' ), $this->plural ),
			'separate_items_with_commas' => sprintf( __( 'Separate %s with commas', 'asr' ), $this->plural ),
			'add_or_remove_items'        => sprintf( __( 'Add or remove %s', 'asr' ), $this->plural ),
			'choose_from_most_used'      => sprintf( __( 'Choose from the most used %s', 'asr' ), $this->plural ),
